...
pub fn get_dataset() -> &'static Mutex<Data> {
    ...
    ONCE.call_once(|| unsafe {
        let df = CsvReader::from_path("backend/data/data-1.csv")
            .unwrap()
            .finish()
            .unwrap();
        let dm = CsvReader::from_path("backend/data/osrm-distance-matrix.csv")
            .unwrap()
            .finish()
            .unwrap();
